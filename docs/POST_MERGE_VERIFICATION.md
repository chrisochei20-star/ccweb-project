...
**Prereqs:** `VITE_API_BASE_URL` on Vercel points at `https://ccweb-api-production-a92c.up.railway.app` (or your live API); the API has `AUTH_JWT_SECRET`, `DATABASE_URL` (if using Postgres features), `CCWEB_ALLOWED_ORIGINS` + `PUBLIC_APP_URL` including the Vercel origin.
