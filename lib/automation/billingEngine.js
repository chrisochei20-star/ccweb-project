...
const { SUBSCRIPTION_PLANS, USAGE_RATES, PERFORMANCE_FEE, LEAD_GEN_RATES } = require("./pricing");
const {
  usageByTenant,
  billingHistory,
  tenantPlans,
...
function ensureUsageBucket(tenantId) {
  ...
  return normalizeUsage(usageByTenant.get(id));
}

function normalizeUsage(bucket) {
  bucket.leadQualified = safeNumber(bucket.leadQualified, 0);
  bucket.outreachActions = safeNumber(bucket.outreachActions, 0);
  bucket.followUpsScheduled = safeNumber(bucket.followUpsScheduled, 0);
  return bucket;
}
...
function trackUsage(tenantId, event) {
  ...
  switch (type) {
    ...
    case "lead_discovered":
      bucket.leadQualified += quantity;
      break;
    case "outreach_action":
      bucket.outreachActions += quantity;
      break;
    case "follow_up_scheduled":
      bucket.followUpsScheduled += quantity;
      break;
    default:
      bucket.apiUnits += quantity;
  }
...
function overageUsd(plan, usage) {
  ...
  const leadsOver = Math.max(0, safeNumber(usage.leadQualified, 0) - safeNumber(limits.includedLeadQualifiedPerMonth, 0));
  const outreachOver = Math.max(0, safeNumber(usage.outreachActions, 0) - safeNumber(limits.includedOutreachActionsPerMonth, 0));
  const followOver = Math.max(0, safeNumber(usage.followUpsScheduled, 0) - safeNumber(limits.includedFollowUpsScheduledPerMonth, 0));
  ...
  return {
    ...
    leadsOverage: leadsOver,
    outreachOverage: outreachOver,
    followUpsOverage: followOver,
    ...
    costLeads: formatMoney(leadsOver * LEAD_GEN_RATES.qualifiedLeadUsd),
    costOutreach: formatMoney(outreachOver * LEAD_GEN_RATES.outreachActionUsd),
    costFollowUps: formatMoney(followOver * LEAD_GEN_RATES.followUpScheduledUsd),
  };
}
...
function calculateCost(tenantId, options = {}) {
  ...
  const usageSubtotal = formatMoney(
    over.costWorkflowRuns +
      over.costAgentActions +
      over.costApi +
      over.costData +
      over.costLeads +
      over.costOutreach +
  ...
  return {
    ...
    usageBreakdown: {
      ...
      actual: {
        ...
        leadQualified: safeNumber(usage.leadQualified, 0),
        outreachActions: safeNumber(usage.outreachActions, 0),
        followUpsScheduled: safeNumber(usage.followUpsScheduled, 0),
      },
      leadGen: {
        included: {
          qualifiedLeads: plan.limits.includedLeadQualifiedPerMonth,
          outreachActions: plan.limits.includedOutreachActionsPerMonth,
          followUps: plan.limits.includedFollowUpsScheduledPerMonth,
        },
        overage: {
          qualifiedLeads: over.leadsOverage,
          outreachActions: over.outreachOverage,
          followUps: over.followUpsOverage,
        },
        chargesUsd: formatMoney(over.costLeads + over.costOutreach + over.costFollowUps),
